...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/download-cv", (req, res) => {
    try {
      ...
      const cvPath = path.join(process.cwd(), "public", "Jana_Kamel_CV.pdf");
      
      // Check if CV file exists, if not return a placeholder response
      if (fs.existsSync(cvPath)) {
        res.download(cvPath, "Jana_Kamel_CV.pdf");
      } else {
        // Create a simple text response indicating where the CV file should be placed
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', 'attachment; filename="Jana_Kamel_CV.pdf"');
        res.status(404).json({ 
          message: "CV file not found. Please place your CV file at /public/Jana_Kamel_CV.pdf" 
        });
      }
    } catch (error) {
